...
public class KafkaToGCS {
  ...
  public static PipelineResult run(KafkaToGCSOptions options) {
    ...
    // Call the function to check File Format passed by user.
    WriteToGCSUtility.checkFileFormatFn(outputFileFormat);
    ...
    PCollection<KV<String, String>> records =
        pipeline
            /*
             * Step #1: Read messages in from Kafka using {@link KafkaIO} and create a PCollection of KV<String, String>.
             */
            .apply(
                "Read From Kafka",
     ...
     */
    records.apply("Write To GCS",
            FileFormatFactory.newBuilder()
                    .setOptions(options)
                    .build());
